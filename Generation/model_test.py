...
class Model(object):
    def __init__(self, opts):
        self.opts = opts
        self.opts.pretrain_model_G = '%s_G.pth' % self.opts.choice
    ...
    def simple_gen(self,number=50):
        ...
        print(self.opts.pretrain_model_G)
    ...
    def load(self, checkpoint_dir):
        ...
        return True, G_epoch
    def save_pc(self, pc, name='experiments', init_num=0):
        cat = str(self.opts.choice).lower()
        # out_folder = os.getcwd() + name + "/%s" % (cat)
        out_folder = os.path.join(*[os.getcwd(), name, cat])
        ...
        if type(pc) == list:
            ...
            if len(dim)==2:
                ...
                if pc.shape[-1] != 3:
                    print('transpose')
                    pc = pc.transpose(1,0)
                file_name = str(0).zfill(4) + "_%s.xyz" % (cat)
                out_file = os.path.join(out_folder, file_name)
                np.savetxt(out_file, pc, fmt="%.6f")
                
            else:
                print('multiple point clouds')
                if pc.shape[-1] != 3:
                    print('transpose')
                    pc = pc.transpose(0,2,1)
                for i in range(dim[0]):
                    file_name = str(init_num + i).zfill(4) + "_%s.xyz" % (cat)
                    out_file = os.path.join(out_folder, file_name)
                    np.savetxt(out_file, pc[i], fmt="%.6f")
