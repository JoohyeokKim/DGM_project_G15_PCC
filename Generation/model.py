...
from Common.loss_utils import AverageValueMeter, dis_loss, gen_loss
...
from Generation.Discriminator import Discriminator, Discriminator_Latent
...
# from metrics.CD_EMD.cd.chamferdist.ChamferDistance import ChamferDistance as chamfer_dist

from testdgm import mass_test, test_completion, test_ae
...
class Model(object):
    ...
    def backup(self):
        if self.opts.phase == 'train':
            ...
            os.system("cp %s/H5DataLoader.py '%s/H5DataLoader.py'" % (source_folder,self.opts.log_dir))
    ...
    def build_model(self):
        ...
        self.train_dataset = CompLoader(self.opts.data_root, category=self.opts.choice)
        ...
        self.optimizerG = optim.Adam(list(filter(lambda p: p.requires_grad, self.Remap.parameters())) + list(filter(lambda p: p.requires_grad, self.G.parameters())), lr=self.opts.lr_g, betas=(beta1, beta2))
        self.optimizerD = optim.Adam(filter(lambda p: p.requires_grad, self.D.parameters()), lr=self.opts.lr_d, betas=(beta1, beta2))
        self.optimizerDL = optim.Adam(filter(lambda p: p.requires_grad, self.DL.parameters()), lr=self.opts.lr_d, betas=(beta1, beta2))
        self.optimizerAE = optim.Adam(list(filter(lambda p: p.requires_grad, self.E.parameters())) + list(filter(lambda p: p.requires_grad, self.G.parameters())), lr=self.opts.lr_d, betas=(beta1, beta2))

        self.optimizerM = optim.Adam(filter(lambda p: p.requires_grad, self.Remap.parameters()), lr=self.opts.lr_g, betas=(beta1, beta2))
    ...
    def sphere_generator(self,bs=2,static=True):
        ...
        if self.ball is None:
            self.ball = np.loadtxt('template/balls/%d_1.xyz'%self.opts.np)[:,:3]
    ...
    def train_ae(self):
        ...
        ae_avg_meter = AverageValueMeter()
        for epoch in range(start_epoch, 2*self.opts.max_epoch+1):
            self.E.train()
            self.G.train()
            # self.D.train()
            losses = []
            for idx, (gt, partial, category) in tqdm(enumerate(self.dataloader, 0),total=len(self.dataloader)):

                self.optimizerAE.zero_grad()
                gt = gt.cuda()

                latent = self.E(gt).unsqueeze(1)
                latent = torch.tile(latent,(1,self.opts.np, 1))
                recon = self.G(x, latent)
                dist1, dist2, idx1, idx2 = chd(gt.permute(0,2,1), recon.permute(0,2,1))
                cdloss = torch.mean(dist1) + torch.mean(dist2)  # find CDLoss to use
                cdloss.backward()
                self.optimizerAE.step()
                losses.append(cdloss.item())
                ae_avg_meter.update(cdloss.item())
            # print("Epoch", epoch, "AE loss:", sum(losses)/len(losses))
            time_tick = time.time() - start_time
            self.log_string("Epoch: [%2d] time: %2dm %2ds,  AE_loss: %.8f" \
                            % (epoch, time_tick / 60, time_tick % 60, ae_avg_meter.avg))
            if epoch % self.opts.snapshot == 0:
                self.save(self.opts.log_dir, epoch, 'ae')
        self.save(self.opts.log_dir, epoch, 'ae')
        if self.opts.choice == 'all':
            for i in self.categories:
                test_ae(self.opts.data_root, self.opts.log_dir, os.path.join(self.opts.log_dir, "results"), epoch, i)
        else:
            test_ae(self.opts.data_root, self.opts.log_dir, os.path.join(self.opts.log_dir, "results"), epoch, self.opts.choice)
    def train_ae_withD(self):
        start_epoch = 1
        x = self.sphere_generator(bs=self.opts.bs)
        chd = chamfer_dist()
        start_time = time.time()
        ...
        ae_avg_meter = AverageValueMeter()
        for epoch in range(start_epoch, self.opts.max_epoch//2+1):
            self.E.train()
            self.G.train()
            # self.D.train()
            losses_g = []
            ...
            for idx, (gt, partial, category) in tqdm(enumerate(self.dataloader, 0),total=len(self.dataloader)):
                ...
                d_real_logit = self.D(gt)
                d_fake_logit = self.D(recon)
                ...
                if idx % 3 == 0:
                    ...
                    g_real_logit = self.D(gt)
                    g_fake_logit = self.D(recon)
                ...
                losses_ae.append(cdloss.item())
                d_avg_meter.update(lossD.item())
                g_avg_meter.update(g_loss.item())
                ae_avg_meter.update(cdloss.item())
            time_tick = time.time() - start_time
            self.log_string("Epoch: [%2d] time: %2dm %2ds d_loss4: %.6f, ae_loss: %.8f, g_loss: %.8f" \
                            % (epoch, time_tick / 60, time_tick % 60,  d_avg_meter.avg, ae_avg_meter.avg, g_avg_meter.avg))
            # self.log_string("Epoch", epoch, "D loss:", sum(losses_d)/len(losses_d), "G loss:", sum(losses_g)/len(losses_g), "AE loss:", sum(losses_ae)/len(losses_ae))
            # self.log_string("real_acc: %f  fake_acc: %f" % (real_acc_avg_meter.avg, fake_acc_avg_meter.avg))
            # print("Epoch", epoch, "D loss:", sum(losses_d)/len(losses_d), "G loss:", sum(losses_g)/len(losses_g), "AE loss:", sum(losses_ae)/len(losses_ae))
            if epoch % self.opts.snapshot == 0:
                self.save(self.opts.log_dir, epoch, 'aed')
        self.save(self.opts.log_dir, epoch, 'aed')
    ...
    def train_lgan(self):
        ...
        for epoch in range(start_epoch, self.opts.max_epoch+1):
            ...
            step_g = 0
            for idx, (gt, partial, category) in tqdm(enumerate(self.dataloader, 0),total=len(self.dataloader)):
                requires_grad(self.Remap, False)
                requires_grad(self.DL, True)
                requires_grad(self.G, False)

                self.optimizerDL.zero_grad()
                gt = gt.cuda()
                partial = partial.cuda()

                # real_points = Variable(gt,requires_grad=True)
                # print(gt.shape)
                z = self.E(gt)
                zp = self.E(partial)
                w = self.Remap(zp)

                d_real_logit = self.DL(z)
                d_fake_logit = self.DL(w)

                # z = torch.tile(z.unsqueeze(1), (1, self.opts.np, 1))
                # recon_r = self.G(x, z)
                # w = torch.tile(w.unsqueeze(1), (1, self.opts.np, 1))
                # recon_f = self.G(x, w)

                # d_real_logit_rec = self.D(recon_r)
                # d_fake_logit_rec = self.D(recon_f)
                # lossD,info= dis_loss(d_real_logit_rec,d_fake_logit_rec,gan=self.opts.gan,noise_label=self.opts.flip_d)


                lossDL,info= dis_loss(d_real_logit,d_fake_logit,gan=self.opts.gan,noise_label=self.opts.flip_d)
                lossDL.backward()
                self.optimizerDL.step()

                # -----------------------------------train G-----------------------------------

                requires_grad(self.Remap, True)
                requires_grad(self.DL, False)
                requires_grad(self.G, True)
                ...
                zp = self.E(partial)
                w = self.Remap(zp)
                g_real_logit = self.DL(z)
                g_fake_logit = self.DL(w)

                lossM,_= gen_loss(g_real_logit,g_fake_logit,gan=self.opts.gan,noise_label=self.opts.flip_d)
                z = torch.tile(z.unsqueeze(1), (1, self.opts.np, 1))
                recon_r_latent = self.G(x, z)
                w = torch.tile(w.unsqueeze(1), (1, self.opts.np, 1))
                recon_f_latent = self.G(x, w)


                dist1_r, dist2_r, idx1_r, idx2_r = chd(gt.permute(0,2,1), recon_r_latent.permute(0,2,1))
                dist1_p, dist2_p, idx1_p, idx2_p = chd(partial.permute(0,2,1), recon_f_latent.permute(0,2,1))
                ae_loss = torch.mean(dist1_r) + torch.mean(dist2_r)
                consistency_loss = torch.mean(dist1_p) + torch.mean(dist2_p)
                loss_rec = lossM + ae_loss + consistency_loss
                loss_rec.backward()
                self.optimizerG.step()

                d_avg_meter.update(lossDL.item())
                m_avg_meter.update(lossM.item())
                ae_avg_meter.update(ae_loss.item())
                g_avg_meter.update(loss_rec.item())

                real_acc_avg_meter.update(info['real_acc'])
                fake_acc_avg_meter.update(info['fake_acc'])

                if self.writer is not  None:
                    self.writer.add_scalar("loss/d_Loss", lossDL.data, global_step)
                    self.writer.add_scalar("loss/m_Loss", lossM.data, global_step)
                    self.writer.add_scalar("loss/tot_Loss", loss_rec.data, global_step)
                    self.writer.add_scalar("loss/recon_Loss", ae_loss.data, global_step)
                    self.writer.add_scalar("loss/consi_Loss", consistency_loss.data, global_step)
            ...
            time_tick = time.time() - start_time
            self.log_string("Epoch: [%2d] time: %2dm %2ds d_loss4: %.6f, m_loss: %.8f, ae_loss: %.8f, g_loss: %.8f" \
                            % (epoch, time_tick / 60, time_tick % 60,  d_avg_meter.avg, m_avg_meter.avg, ae_avg_meter.avg, g_avg_meter.avg))
            self.log_string("real_acc: %f  fake_acc: %f" % (real_acc_avg_meter.avg, fake_acc_avg_meter.avg))
            ...
            if epoch % self.opts.snapshot == 0:
                self.save(self.opts.log_dir, epoch, 'lgan')
        ...
        if self.opts.choice == 'all':
            for i in self.categories:
                test_completion(self.opts.data_root, self.opts.log_dir, os.path.join(self.opts.log_dir, "results"), epoch, i)
        else:
            test_completion(self.opts.data_root, self.opts.log_dir, os.path.join(self.opts.log_dir, "results"), epoch, self.opts.choice)

    def train_lgan_only_latent(self):
        self.build_model()
        self.backup()
        if self.opts.restore:
            could_load, save_epoch = self.load(self.opts.log_dir)
        ...
        for epoch in range(start_epoch, self.opts.max_epoch+1):
            ...
            for idx, (gt, partial, category) in tqdm(enumerate(self.dataloader, 0),total=len(self.dataloader)):
                ...
                ae_loss = torch.mean(dist1_r) + torch.mean(dist2_r)
                consistency_loss = torch.mean(dist1_p) + torch.mean(dist2_p)
                loss_rec = lossM + ae_loss + consistency_loss*100
                loss_rec.backward()
    ...
    def train_ae_latent_const(self):
        start_epoch = 1
        x = self.sphere_generator(bs=self.opts.bs)
        chd = chamfer_dist()
        start_time = time.time()
        ae_avg_meter = AverageValueMeter()
        l_avg_meter = AverageValueMeter()
        for epoch in range(start_epoch, 2*self.opts.max_epoch+1):
            self.E.train()
            self.G.train()
            # self.D.train()
            ...
            for idx, (gt, partial, category) in tqdm(enumerate(self.dataloader, 0),total=len(self.dataloader)):
                ...
                latent = self.E(gt).unsqueeze(1)
                recon = self.G(x, torch.tile(latent,(1,self.opts.np, 1)))
                latent_recon = self.E(recon).unsqueeze(1)
                # latent_recon = torch.tile(latent_recon, (1, self.opts.np,1))
                latent_loss = torch.nn.functional.l1_loss(latent, latent_recon)
                dist1, dist2, idx1, idx2 = chd(gt.permute(0,2,1), recon.permute(0,2,1))
                cdloss = torch.mean(dist1) + torch.mean(dist2)  # find CDLoss to use
                tot_loss = cdloss + latent_loss
                tot_loss.backward()
                self.optimizerAE.step()
                # losses.append(cdloss.item())
                ae_avg_meter.update(cdloss.item())
                l_avg_meter.update(latent_loss.item())
            # print("Epoch", epoch, "AE loss:", sum(losses)/len(losses))
            time_tick = time.time() - start_time
            self.log_string("Epoch: [%2d] time: %2dm %2ds,  AE_loss: %.8f, Latent_loss: %.8f" \
                            % (epoch, time_tick / 60, time_tick % 60, ae_avg_meter.avg, l_avg_meter.avg))
            if epoch % self.opts.snapshot == 0:
                self.save(self.opts.log_dir, epoch, 'ae')
        self.save(self.opts.log_dir, epoch, 'ae')
        if self.opts.choice == 'all':
            ...
            test_ae(self.opts.data_root, self.opts.log_dir, os.path.join(self.opts.log_dir, "results"), epoch, self.opts.choice)
    def train_lgan_latent_const(self):
        self.build_model()
        self.backup()
        if self.opts.restore:
            could_load, save_epoch = self.load(self.opts.log_dir)
        ...
        for epoch in range(start_epoch, self.opts.max_epoch+1):
            ...
            for idx, (gt, partial, category) in tqdm(enumerate(self.dataloader, 0),total=len(self.dataloader)):
                ...
                # z = 
                # recon_r = self.G(x, torch.tile(z.unsqueeze(), (1, self.opts.np, 1)))
                # w = 
                ...
                lossM,_= gen_loss(g_real_logit,g_fake_logit,gan=self.opts.gan,noise_label=self.opts.flip_d)
                recon_r_latent = self.G(x, torch.tile(z.unsqueeze(1), (1, self.opts.np, 1)))
                recon_f_latent = self.G(x, torch.tile(w.unsqueeze(1), (1, self.opts.np, 1)))
                latent_fake = self.E(recon_f_latent)
                latent_loss = torch.nn.functional.l1_loss(z, latent_fake)


                dist1_r, dist2_r, idx1_r, idx2_r = chd(gt.permute(0,2,1), recon_f_latent.permute(0,2,1))
                dist1_p, dist2_p, idx1_p, idx2_p = chd(partial.permute(0,2,1), recon_f_latent.permute(0,2,1))
                ae_loss = torch.mean(dist1_r) + torch.mean(dist2_r)
                consistency_loss = torch.mean(dist1_p) + torch.mean(dist2_p)
                loss_rec = lossM + ae_loss + consistency_loss + latent_loss
                loss_rec.backward()
                self.optimizerM.step()
            ...
            time_tick = time.time() - start_time
            self.log_string("Epoch: [%2d] time: %2dm %2ds d_loss4: %.6f, m_loss: %.8f, ae_loss: %.8f, g_loss: %.8f, l_loss: %.8f" \
                            % (epoch, time_tick / 60, time_tick % 60,  d_avg_meter.avg, m_avg_meter.avg, ae_avg_meter.avg, g_avg_meter.avg, l_avg_meter.avg))
            self.log_string("real_acc: %f  fake_acc: %f" % (real_acc_avg_meter.avg, fake_acc_avg_meter.avg))
            # self.log_string("lr_g: %f  lr_d: %f" % (self.optimizerG.param_groups[0]['lr'], self.optimizerD.param_groups[0]['lr']))
            # print("step_d:%d step_g:%d"%(step_d,step_g))
            # if self.scheduler_G is not None and self.scheduler_D is not None:
    ...
    def draw_sample_save_AE(self, epoch, step, category):
        ...
        x = self.sphere_generator(bs=grid_y)
        category_set = CompLoader(self.opts.data_root, category)
        category_loader = torch.utils.data.DataLoader(category_set, shuffle=False, batch_size=1)


        pcds_list = []
        title_list = []
        for i, (gt, partial, category) in enumerate(category_loader):
            title = ["S_%d" % (i * grid_y + j) for j in range(grid_y)]
            with torch.no_grad():
                #z = self.noise_generator(bs=grid_y)
                latent = self.E(gt).unsqueeze(1)
                latent = torch.tile(latent,(1,self.opts.np, 1))
                out_pc = self.G(x, latent)
                out_pc = out_pc.transpose(2, 1)
                sample_pcs = out_pc.cpu().detach().numpy()
                sample_pcs = normalize_point_cloud(sample_pcs)
        ...
        plot_name = os.path.join(eval_dir, "AE_"+str(step)+category + ".png")
    ...
    def load_AE(self, checkpoint_dir):
        checkpoint = torch.load(os.path.join(checkpoint_dir, ""))
        self.E.load_state_dict(checkpoint['E_model'])
    ...
    def save(self, checkpoint_dir, index_epoch, phase):
        if not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir)
        save_name = str(index_epoch)+'_'+phase#+"_"+self.opts.choice
        path_save_G = os.path.join(checkpoint_dir, save_name+'_G.pth')
        path_save_M = os.path.join(checkpoint_dir, save_name+'_M.pth')
        path_save_E = os.path.join(checkpoint_dir, save_name+'_E.pth')
        path_save_D = os.path.join(checkpoint_dir, save_name+'_D.pth')
        path_save_DL = os.path.join(checkpoint_dir, save_name+'_DL.pth')
        print('Save Path for G: {}'.format(path_save_G))
        print('Save Path for D: {}'.format(path_save_D))
        print('Save Path for M: {}'.format(path_save_M))
        print('Save Path for E: {}'.format(path_save_E))
        print('Save Path for DL: {}'.format(path_save_DL))
        torch.save({
        ...
        torch.save({
                'E_model':self.E.module.state_dict() if self.multi_gpu else self.E.state_dict(),
                'E_optimizer': self.optimizerAE.state_dict() ,
                'E_epoch': index_epoch,
            }, path_save_E)
        torch.save({
                'remap_model':self.Remap.module.state_dict() if self.multi_gpu else self.Remap.state_dict(),
                'remap_optimizer': self.optimizerG.state_dict() ,
                'remap_epoch': index_epoch,
            }, path_save_M)
        torch.save({
        ...
        torch.save({
                ...
                # 'D_model2': self.discriminator2.state_dict(),
                # 'D_model3': self.discriminator3.state_dict(),
                'DL_model': self.DL.module.state_dict() if self.multi_gpu else self.DL.state_dict(),
                'DL_optimizer': self.optimizerDL.state_dict(),
                # 'D_optimizer2': self.optimizerD2.state_dict(),
                # 'D_optimizer3': self.optimizerD3.state_dict(),
                # 'D_optimizer4': self.optimizerD.state_dict(),
                'DL_epoch': index_epoch,
            }, path_save_DL)
